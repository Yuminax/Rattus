...
public class reception_verification : MonoBehaviour
{
    ...
    [SerializeField] GameObject lightWhite;
    [SerializeField] GameObject lightBlue;
    ...
    void Update()
    {
        ...
        if (lastClicked != null)
        {
            ...
            {
                dialogue.AddSentence("Moi", "Le cable électrique semble cassé, il faut que je le répare", 6);
                FindObjectOfType<DialogueManager>().StartDialogue(dialogue, blockerPlane);
            }
            else if (lastClicked.name == "PC_Monitor" && Conditions["RepairedCable"])
            {
