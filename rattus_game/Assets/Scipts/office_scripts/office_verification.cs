        ...
        void Update()
        {
            ...
            if (lastClicked != null)
            {
                ...
                if (!Conditions["ReadLetter"] && (lastClicked.name == "Board" || lastClicked.name == "PAGE" || lastClicked.name == "clipboardText"))
                {
                    ...
                    dialogue.AddSentence("Lettre", "vous avez dit que ça l'aiderait,", 4);
                    dialogue.AddSentence("Lettre", "qu'il n'y avait que quelques risques d'échecs.", 7);
                    dialogue.AddSentence("Lettre", "Ce que vous avez omis de dire, c'était le nom de cette opération.", 8);
                    dialogue.AddSentence("Lettre", "Je l'ai par la suite appris par l'une des infirmières,", 6);
